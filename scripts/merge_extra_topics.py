...
def merge_extra_topics(topics_folder, output_file):
    merged_data = []
    ...
    for filename in json_files:
        ...
        try:
            ...
            print(f"Merged Extra topic: {topic_name} ({len(questions)} questions)")
            
        except Exception as e:
            print(f"Error merging {filename}: {str(e)}")
            
    # Write the combined data to the final extra_questions.json
    with open(output_file, 'w', encoding='utf-8') as out_f:
        json.dump(merged_data, out_f, indent=2, ensure_ascii=False)
        
    print(f"\nSuccessfully merged {len(merged_data)} Extra topics into {output_file}")

if __name__ == "__main__":
    merge_extra_topics('data/extra_topics', 'data/extra_questions.json')
